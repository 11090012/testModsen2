...
function restartQuiz() {
  ...
  // Установить начальное значение таймера и запустить его при перезапуске теста
...
function startTimer() {
  timerInterval = setInterval(() => {
    ...
    if (remainingTime <= 0) {
      // Удалить этот блок, чтобы не спрашивать пользователя, хочет ли он закончить
      // confirmEndQuiz();

      // Закончить квиз сразу, когда время истекает
      showResults();
    }
  }, 1000);
}
...
// Установить начальное значение таймера и запустить его при загрузке страницы
remainingTime = questions.length * 10;
updateTimer();
startTimer();
